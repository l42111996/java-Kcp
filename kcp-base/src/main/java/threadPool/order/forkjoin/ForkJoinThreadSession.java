 ...
 * 2020/6/19.
 */
public class ForkJoinThreadSession implements Runnable{
    ...
    public ForkJoinThreadSession() {
        this(new MpscLinkedQueue<>());
    }

    public ForkJoinThreadSession(Queue<Runnable> queue) {
        this(queue, Integer.MAX_VALUE);
    }

    public ForkJoinThreadSession(Queue<Runnable> queue, int runTaskCount) {
        this.queue = queue;
    ...
    @Override
    public void run() {
        Queue<Runnable> tasksQueue = getQueue();
        Runnable task;
        for (; ; ) {
            //if(runTaskCount--==0){
            //    break;
