...
/**
 */
public class EKcp implements IKcp {

    private static final InternalLogger log = InternalLoggerFactory.getInstance(EKcp.class);
    ...
    private static void output(ByteBuf data, EKcp kcp) {
    ...
    private static int encodeSeg(ByteBuf buf, Segment seg) {
        ...
        ByteBufUtils.writeInt(buf,seg.conv);
        ByteBufUtils.writeByte(buf,seg.cmd);
        ByteBufUtils.writeByte(buf,seg.frg);
        ByteBufUtils.writeShort(buf,seg.wnd);
        ByteBufUtils.writeInt(buf,(int) seg.ts);
        ByteBufUtils.writeInt(buf,(int) seg.sn);
        ByteBufUtils.writeInt(buf,(int) seg.una);

        int dataSize = seg.data==null?0:seg.data.readableBytes();

        ByteBufUtils.writeInt(buf,dataSize);
        switch (seg.ackMaskSize){
            case 8:
                ByteBufUtils.writeByte(buf, (int) seg.ackMask);
                 break;
            case 16:
                ByteBufUtils.writeShort(buf, (int) seg.ackMask);
                break;
            case 32:
                ByteBufUtils.writeInt(buf, (int) seg.ackMask);
                break;
            case 64:
                ByteBufUtils.writeLong(buf, (int) seg.ackMask);
                break;
        }
    ...
    public EKcp(int conv, KcpOutput output) {
        this.conv = conv;
        this.output = output;
    }
    ...
    public int input(ByteBuf data, boolean regular, long current) {
        ...
        while (true) {
            ...
            conv = ByteBufUtils.readInt(data);
            if (conv != this.conv) {
                return -4;
            }

            cmd = ByteBufUtils.readByte(data);
            frg = ByteBufUtils.readUByte(data);
            wnd = ByteBufUtils.readUShort(data);
            ts = ByteBufUtils.readUInt(data);
            sn = ByteBufUtils.readUInt(data);
            una = ByteBufUtils.readUInt(data);
            len = ByteBufUtils.readInt(data);


            switch (ackMaskSize){
                case 8:
                    ackMask = ByteBufUtils.readByte(data);
                    break;
                case 16:
                    ackMask = ByteBufUtils.readShort(data);
                    break;
                case 32:
                    ackMask = ByteBufUtils.readInt(data);
                    break;
                case 64:
                    //TODO need unsignedLongLe
                    ackMask = ByteBufUtils.readLong(data);
                    break;
                default:
                    ackMask=0;
            }
    ...
    public static class Segment {
        ...
        private final Recycler.Handle<EKcp.Segment> recyclerHandle;
        /**会话id**/
        private int conv;
        /**命令**/
        private byte cmd;
        ...
        private static final Recycler<EKcp.Segment> RECYCLER = new Recycler<EKcp.Segment>() {
        ...
        private Segment(Recycler.Handle<EKcp.Segment> recyclerHandle) {
